...
class idex (Exchange):
    ...
    async def idex_order(self, base, quote, side, amount, price, nonce, params={}):
        ...
        return await self.privatePostOrder(request)
    ...
    async def idex_trade(self, base, quote, side, amount, nonce, params={}):
        amountFloat = float(amount)
        ...
        orderbook = await self.publicGetReturnOrderBook(request)
        orderbookKey = None
        if side == 'buy':
            orderbookKey = 'asks'
        elif side == 'sell':
    ...
    async def fetch_next_nonce(self):
        nonce = None
        nonceResponse = await self.privatePostReturnNextNonce({'address': self.walletAddress})
    ...
    async def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        if (uppercaseType == 'LIMIT') and(price is not None):
            response = await self.idex_order(base, quote, side, amount, price, nonce, params)
            result = self.parse_order(response, market)
        elif uppercaseType == 'MARKET':
            response = await self.idex_trade(base, quote, side, amount, nonce, params)
            result = []
    ...
    async def cancel_order(self, orderHash, symbol=None, params={}):
        nonce = await self.fetch_next_nonce()
        args = {
            'orderHash': orderHash,
            'nonce': nonce,
        ...
        request = self.get_signed_request_params(args, raw)
        return await self.privatePostCancel(request)

    async def withdraw(self, code, amount, address=None, tag=None, params={}):
        ...
        request = self.get_signed_request_params(args, raw)
        return await self.privatePostWithdraw(request)
